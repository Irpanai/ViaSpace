...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $totalIzin = Attendance::where('user_id', $user->id)->where('status', 'permission')->count();
        $totalSakit = Attendance::where('user_id', $user->id)->where('status', 'sick')->count();
        ...
        $topInterns = \App\Models\User::where('role', 'intern')->get()->sortByDesc('monthly_points')->take(5);

        return view('intern.dashboard', compact('hasSchedule', 'attendance', 'today', 'startTime', 'endTime', 'totalHadir', 'totalIzin', 'totalSakit', 'officePolygon', 'officeLat', 'officeLng', 'todayHoliday', 'topInterns'));
    }
